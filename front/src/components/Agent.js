...
class Agent extends React.Component {
    ...
    getTableData = async (agentName) => {
        ...
        if(unconfirmedArr.length>unconfirmedLength){
            ...
            document.title = "Sophos Home Scheduler"
        )
        ...
        return (<div>
            <Card style={{width: "80%", margin: "2% auto"}}>
                <Grid container spacing={2}>
                    ...
                    <Grid item xs={4} md={4}>
                        <Item><TextField label='Search ticket#' type="search" placeholder='Ticket #' onChange={this.handleSearchChange} size='small'/></Item>
                    </Grid>
                    <Grid item xs={4} md={4}>
                        <Item><button style={{borderWidth:'0',color:'#E4A11B'}} onClick={this.handleSignOut}>Sign out</button></Item>
                    </Grid>
                </Grid>
                <Item hidden={hidden}><Typography>No data found</Typography></Item>
                        ...
                        <TableBody>
                            {rows.map((row) => (
                                ...
                                >
                                    ...
                                    <TableCell align="center">{row.userTime + " (" + row.timeZone + ")"}</TableCell>
            ...
            >
                <Box sx={style}>
                    <Typography id="modal-modal-description" sx={{mt: 2}}>
                        Remote confirmation email sent to tkt#{rowData ? rowData.ticketNum : ""}
                    </Typography>
